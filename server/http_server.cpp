...
int Server::ListenAndServe(int port, int nthreads, int backlog, IHandler *handler)
{
...
int Server::ensure_listen(int port, int backlog) 
{
  int r;
  int nfd;
...
void Server::event_cb(struct evhttp_request *req, void *arg)
{
    ...
    std::map<std::string, IHandler*>::const_iterator iter = server->mux_.find(uri);
